...
export function useBreadcrumbs(): BreadcrumbSegment[] {
  ...
  if (path.startsWith("/vc/")) {
    const vcSlug = params.vcSlug;
    const vcName = vcSlug ? vcSlug.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()) : "Virtual Contributor";
    return [
      { label: vcName, href: `/vc/${vcSlug}`, isCurrentPage: true },
    ];
  }
